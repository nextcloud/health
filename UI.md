...
The main Statistics workspace keeps editable metric and period controls and offers Save view. A saved Statistics view is a private, deep-linkable child of Statistics in the application navigation. It displays its title, icon, stored metrics, and period read-only; Edit is the only way to change its configuration. Statistics is a normal navigation item without an expander when no saved views exist. Saved-view navigation menus offer Clone and Delete, while the opened-view header menu offers Edit and Delete; delete always requires confirmation. Create, edit, and clone use the supported Nextcloud native-emoji picker for the view icon. Create and Edit display icon and title together, followed vertically by full-width Period and multi-line Metrics controls.
...
The same Integration section exposes Open / Set up PWA and Reset PWA connection actions. The integrated Health PWA is a responsive, touch-first capture surface with semantic large buttons, visible keyboard focus, accessible metric names, browser zoom support, and reduced-motion handling. Its home is a responsive grid of equal-sized icon-and-label metric launchers with no recorded values. Shared hydration metadata renders Water as a direct `small_glass` action and Coffee as a launcher for the supported coffee-type buttons; selecting a coffee type records or queues it immediately and closes the dialog. Other launcher controls derive from capability metadata: small finite scales and named events use immediate buttons; daily counters provide both an explicit numeric Save flow and an immediate `+1`; arbitrary numeric and composite values retain focused inputs and require Save. Immediate actions record or queue with a short per-metric action lock, then close the dialog. The PWA never uses an intermediate option select for those choices.

The PWA header follows the compact Taskbook pattern: theme-appropriate Health icon and title, an accessible sync icon control, and a three-dot device-actions dialog. The dialog offers disconnect (with an explicit pending-entry warning) and technical diagnostics. Diagnostics are a copyable technical allow-list containing build version, server origin, online/authentication/synchronization state, last sync time, generic pending count, and service-worker state. They never include Health values, goals, journal content, or credentials. A subtle offline notice says that entries will sync later; authentication, server, and repeated synchronization failures receive an alert below the header. Status text must not display current values, history, goals, Statistics, charts, totals, trends, or interpretation.

Today's Journal combines applicable daily goal progress with current open-ended long-term progress. A metric may show multiple applicable goal rows. An applicable long-term Weight goal is rendered beside the Weight value as an accessible `NcProgressBar` plus baseline/current/target supporting text. Its server-derived journey ratio advances toward a lower `lte` target or a higher `gte` target and is clamped from zero to complete.
