...
`POST /entries` and `POST /measurements` accept an optional UUID v4 `operationId`. Repeating a request for the same authenticated owner and metric with the same operation ID returns the existing logical record instead of creating a duplicate. Reusing it for another metric is rejected. The field supports offline replay; it is not an owner selector and is not included in response resources. Daily values remain idempotent by their existing owner/metric/local-date PUT identity.
